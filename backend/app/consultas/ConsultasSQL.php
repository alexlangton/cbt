...
class ConsultasSQL {
    ...
    protected function logQuery($sql, $params = []) {
        try {
            ...
            // Registrar la consulta SQL utilizando el Logger
            $this->logger->sql($queryFinal, $datosLog);

        } catch (\Exception $e) {
            // Si hay un error en el logging, lo registramos como error utilizando el Logger
            $this->logger->error('Error en logQuery: ' . $e->getMessage());
        }
    }
